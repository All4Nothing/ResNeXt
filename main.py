...
base_wdith = 64  # 기초 채널 수


# (c)의 grouped convolution 레이어는 입출력 채널이 4차원인 32개의 컨볼루션 그룹(=cardinality)를 형성한다.
...
class ResNextBottleNeck(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, stride):
        ...
        super().__init__()  # 부모 클래스(nn.Module)의 성 및 메소드를 불러온다.

        groups = cardinality  # 특징 맵(feature map)이 분할된 그룹 수

        # 그룹당 채널 수(depth per group)
        num_depth = int(depth * out_channels / base_width)
        self.split_transform = nn.Sequential(
            nn.Conv2D(in_channels, groups * num_depth,
                      kernel_size=1, groups=groups, bias=False),
            nn.BatchNorm2d(groups * num_depth),
            nn.ReLU(),
            nn.Conv2D(groups * num_depth, groups * num_depth, kernel_size=3,
                      stride=stride, groups=groups, padding=1, bias=False),
            nn.BatchNorm2d(groups * num_depth),
            nn.ReLU(),
            nn.Conv2D(groups * num_depth, out_channels *
        ...
        if stride != 1 or in_channles != out_channels * 4:
            self.shortcut = nn.Sequential(
                nn.Conv2d(in_channles, out_channels * 4,
...
class ResNext(nn.Module):
    ...
    def _make_layer(self, block, num_block, out_channels, stride):
        ...
        return nn.Sequential(*layers) # positional arguments만 받음
